...
public class VideoViewActivity extends AppCompatActivity implements View.OnTouchListener,View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState){
        ...
        init();
        mFirebaseAnalytics = FirebaseAnalytics.getInstance(VideoViewActivity.this);

        alertVault = new AlertDialog.Builder(this).create();
        LayoutInflater factory = LayoutInflater.from(VideoViewActivity.this);
        ...
        from = getIntent().getStringExtra("Video");
        position = getIntent().getIntExtra("Position", 0);

        from = getIntent().getStringExtra("from");
        directoryPosition = getIntent().getIntExtra("directoryPosition", 0);
        createAlbumAdapter = new CreateAlbumAdapter(dateFiles, VideoViewActivity.this, new CreateAlbumAdapter.ItemClickListener() {
            @Override
        ...
        rvListView.setLayoutManager(new LinearLayoutManager(VideoViewActivity.this, RecyclerView.VERTICAL, false));
        rvListView.setAdapter(createAlbumAdapter);
        new LoadVideos(VideoViewActivity.this).execute();
        new LoadAlbumImages(VideoViewActivity.this).execute();
        ...
        switch (v.getId()) {
            ...
            case R.id.option4:
                if (SharedPreference.getPasswordProtect(getBaseContext()).equals("")) {
                    Intent in=new Intent(VideoViewActivity.this, VaultActivity.class);
                    ...
                    if (file1.getAbsolutePath().contains(sampleFile.getAbsolutePath())) {
                        ...
                        if (!SharedPreference.getSharedPreference(VideoViewActivity.this).contains(file1.getParentFile().getAbsolutePath())) {
            ...
            case R.id.imgShare:{
                ...
                Uri contentUri = FileProvider.getUriForFile(VideoViewActivity.this,getPackageName() + ".provider",file);
            ...
            public void onClick(View v){
                ...
                if (file1.getAbsolutePath().contains(sampleFile.getAbsolutePath())) {
                    if (sourceFile.exists()){
                        Util.MoveToTrash(VideoViewActivity.this,sourceFile);
                        isDelete = Util.delete(VideoViewActivity.this, sourceFile);
...
//                    //   Log.e("LLL_name: ",data.getString(data.getColumnIndexOrThrow(MediaStore.Video.Media.TITLE)));
        ...
        protected void onPostExecute(List<Directory<BaseModel>> directories) {
            ...
            privateList = SharedPreference.getHideFileList(VideoViewActivity.this);
...
//                //   Log.e("privateList i" + i,privateList.get(i));
            ...
            demoAdapter = new VideoViewAdapter(VideoViewActivity.this,videoFiles,finalPosition,"All");
            LinearLayoutManager horizontalLayoutManagaer
                    = new LinearLayoutManager(VideoViewActivity.this, LinearLayoutManager.HORIZONTAL, false);
    ...
    public void moveData(File sourceFile ,String targetName,File destinationFile){
        try{
            ...
            if(to.getPath().equals(sourceFile.getPath())){
                Toast.makeText(VideoViewActivity.this,"Already in same folder", Toast.LENGTH_SHORT).show();
            }else {
...
//            if (sourceFile.exists()) {
                boolean isDelete = Util.delete(VideoViewActivity.this, sourceFile);
                if (!isDelete)
...
//            }
                ...
                if (isDelete == true && isRename == true) {
                    ...
                    handler.postDelayed(new Runnable() {
                        ...
                        public void run() {
                            Util.IsUpdate = true;
                            Util.IsAlbumUpdate = true;
                            LocalBroadcastManager lbm = LocalBroadcastManager.getInstance(VideoViewActivity.this);
    ...
    public DocumentFile getDocumentFile(final File file, final boolean isDirectory) {
        ...
        Uri treeUri = Uri.parse(SharedPreference.getSharedPreferenceUri(VideoViewActivity.this));
        ...
        DocumentFile document = DocumentFile.fromTreeUri(VideoViewActivity.this, treeUri);
...
//       Log.e("To path",to.getPath());
        ArrayList<String> hideFileList = SharedPreference.getHideFileList(VideoViewActivity.this);
        hideFileList.add(to.getAbsolutePath());
        SharedPreference.setHideFileList(VideoViewActivity.this, hideFileList);

        try {
                boolean isDelete=samplefile.delete();
                if(!isDelete)
                    isDelete=Util.delete(VideoViewActivity.this,samplefile);
                super.onBackPressed();

            scanPhoto(from.toString());
    ...
    private void getData(File file1) {
        ...
        try {
            ...
            if (file1.exists()) {
                boolean isDelete = Util.delete(VideoViewActivity.this, file1);
            }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent resultData) {
        ...
        if (resultCode == RESULT_OK) {
            ...
            if (documentFile == null) {
                ...
                SharedPreference.setSharedPreferenceUri(VideoViewActivity.this, documentFile1.getUri());
                SharedPreference.setSharedPreference(VideoViewActivity.this, file.getParentFile().getAbsolutePath());
                ...
                SharedPreference.setSharedPreferenceUri(VideoViewActivity.this, documentFile.getUri());
                SharedPreference.setSharedPreference(VideoViewActivity.this, file1.getParentFile().getAbsolutePath());
