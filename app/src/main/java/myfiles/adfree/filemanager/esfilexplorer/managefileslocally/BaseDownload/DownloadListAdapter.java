...
public class DownloadListAdapter extends RecyclerView.Adapter<DownloadListAdapter.MyClassView> {
    ...
    public DownloadListAdapter(ArrayList<BaseModel> images,Activity activity) {
    ...
    @NonNull
    @Override
    public DownloadListAdapter.MyClassView onCreateViewHolder(@NonNull ViewGroup parent,int viewType) {
        View itemView = LayoutInflater.from(activity).inflate(R.layout.trash_layout, parent, false);
        return new DownloadListAdapter.MyClassView(itemView);
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    @Override
    public void onBindViewHolder(@NonNull DownloadListAdapter.MyClassView holder,int position) {
        ...
        if(Util.clickEnable == true){
            holder.itemView.setOnClickListener(v -> {
                ...
                if(file.getPath().toLowerCase().endsWith(".png") || file.getPath().toLowerCase().endsWith(".jpg") || file.getPath().toLowerCase().endsWith(".jpeg")){
                    ...
                    unZipPAth=file.getPath();
                    Intent theIntent = new Intent(Intent.ACTION_VIEW);
                    theIntent.setDataAndType(Uri.fromFile(new File(path1)),"application/zip");
...
//        if(mSelectedDownloadList.contains(file)){
//            holder.mSelect.setVisibility(View.VISIBLE);
//            holder.mDeselect.setVisibility(View.GONE);
//        }
